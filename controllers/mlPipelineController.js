    ...
    for (const [userId, rfpArray] of Object.entries(nestedRFPs)) {
      ...
      for (const rfp of rfpArray) {
        transformedData.push({
          ...
          fundingType: 'Government',
          organizationType: rfp['Industry'] || "",
          link: rfp['URL'] || `${process.env.APP_URL}/profile/getDocument/${req.file.id}`,
          type: 'Matched',
          contact: rfp['Contact Information'] || "",
          timeline: rfp['Timeline'] || "",
          email: companyProfile_1.email || ""
    ...
    try {
        ...
        if (resultKeys.length > 0) {
          ...
          if (structuredData && structuredData.structured_fields) {
            ...
            rfp = {
              ...
              link: fields['url'] || `${process.env.APP_URL}/profile/getDocument/${req.file.id}`,
      ...
      if (!rfp) {
        rfp = {
          ...
          link: `${process.env.APP_URL}/profile/getDocument/${req.file.id}`,
      ...
      const newRFP = await MatchedRFP.create({
        ...
        organization: rfp.organization || "",
        organizationType: rfp.organizationType || "",
        link: rfp.link || `${process.env.APP_URL}/profile/getDocument/${req.file.id}`,
        email: userEmail,
        budget: rfp.budget || 'Not found',
        deadline: rfp.deadline || "",
        contact: rfp.contact || "",
  ...
  async (req, res) => {
    try {
      ...
      grant = {
        ...
        OPPORTUNITY_NUMBER_LINK: grant.OPPORTUNITY_NUMBER_LINK || `${process.env.APP_URL}/profile/getDocument/${req.file.id}`,
        ...
        LINK_TO_ADDITIONAL_INFORMATION: grant.LINK_TO_ADDITIONAL_INFORMATION || `${process.env.APP_URL}/profile/getDocument/${req.file.id}`,
